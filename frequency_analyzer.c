...
void BanList(wchar_t*** words, int** counts, int* size, const char* path, wchar_t*** newWords, int** newCounts, int* newSize) {
    ...
    *newWords = (wchar_t**)malloc(*size * sizeof(wchar_t*));
    *newCounts = (int*)malloc(*size * sizeof(int));
    *newSize = 0;
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 5) {
        fwprintf(stderr, L"Використання: %s <input> <banlist> <result> <result1>\n", argv[0]);
        return 1;
    }
    ...
    int* filteredCounts;
    int filteredSize;
    BanList(&words, &counts, &size, argv[2], &filteredWords, &filteredCounts, &filteredSize);

    SaveResult(filteredWords, filteredCounts, filteredSize, argv[3], argv[4]);
